...
CSvmTrain::CSvmTrain(){
	string strSVMPath = "C:\\Users\\work\Desktop\\";
}
CSvmTrain::~CSvmTrain(){
...
void CSvmTrain::SvmTrain(cv::Mat HogFeature,cv::Mat HogResult,int intPosNum,int intNegNum,cv::Size windSize,cv::Size cellSize){
	...
	//Mat tempHogFeature(HogFeature.size(), CV_32FC1);
	//Mat tempHogResult(HogResult.size(), CV_32FC1);
	//HogFeature.convertTo(tempHogFeature, CV_32FC1,255,255);
	//HogResult.convertTo(tempHogResult, CV_32FC1,255,255);
	//if (HogFeature.type()==CV_32FC1)
	//{
	//	cout << "yes" << endl;
	//}
